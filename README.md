...
本项目使用 **Atlas** 进行数据库迁移管理。Atlas 是 Ent 官方推荐的迁移工具，支持版本化迁移和自动化部署。

> **⚠️ 重要**: 由于项目使用 Go workspace 模式，所有 Atlas 命令需要添加 `GOWORK=off` 前缀。为简化操作，建议使用提供的自动化工具。
...
**⚠️ 注意**：自动迁移不会生成迁移历史文件，不适合生产环境和团队协作。
