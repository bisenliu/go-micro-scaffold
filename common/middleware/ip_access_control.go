...
func IPWhitelistMiddleware(allowedIPs []string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if !exists {
			...
			err := response.NewForbiddenError("Access denied: Internal error")
		...
		if !allowed {
			...
			err := response.NewForbiddenError("Access denied")
			response.Handle(c, nil, err)
			c.Abort()
			return
		}
...
func InternalIPMiddleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if !isPrivateIP(ip, privateIPBlocks) {
			...
			err := response.NewForbiddenError("Access denied: Internal network only")
			response.Handle(c, nil, err)
			c.Abort()
			return
		}
